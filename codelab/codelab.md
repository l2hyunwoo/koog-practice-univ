...
| `simpleGoogleAIExecutor` | Google Gemini API와 연결하는 실행기 | LLM과 통신하는 API 클라이언트 |
...
- **`simpleGoogleAIExecutor(apiKey)`**: API Key로 Gemini에 연결합니다. 에이전트가 LLM과 통신할 수 있게 해주는 클라이언트입니다.
...
| `Command` 인터페이스 | 슬래시 명령어를 구조적으로 처리하는 패턴 | 새 명령어를 추가할 때 기존 코드를 수정하지 않아도 된다 |
